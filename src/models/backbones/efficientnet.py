"""
EfficientNetB0-Fingerprint backbone — best accuracy/efficiency tradeoff.
"""
...
def build_efficientnet_fingerprint(num_classes: int,
                                    dense_units: int = 1024,
                                    dropout: float = 0.5,
                                    img_size: tuple = (224, 224)) -> Model:
    """
    EfficientNetB0 (ImageNet) → fingerprint classification head.

    Note: EfficientNet includes its own rescaling layer, so do NOT apply
    ...
    return Model(inputs=base.input, outputs=out, name="EfficientNetB0_Fingerprint")
